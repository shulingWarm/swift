...
class Template(ProcessorMixin):
    ...
    def __init__(
        ...
        *,
        ...
        # 默认情况下tokenizer functor被指定成None
        self.tokenizer_functor = None
    ...
    # 注册tokenizer的functor
    def register_tokenizer_functor(self, tokenizer_functor):
        # 记录tokenizer functor
        self.tokenizer_functor = tokenizer_functor
        # 注册相关的内置tokenizer
        if (self.tokenizer_functor is not None):
            self.tokenizer_functor.register_tokenizer(self.tokenizer)
            self.tokenizer_functor.register_loss_scale(self.loss_scale)
    ...
    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        ...
        if self.is_encoder_decoder or self.mode == 'gkd':
            ...
            # 如果有tokenizer functor，那就使用tokenizer functo来处理
            if (self.tokenizer_functor is not None):
                input_ids, labels, loss_scale, tokenizer_kwargs = self.tokenizer_functor.generate_input_id(
