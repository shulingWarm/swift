...
class SwiftSft(SwiftPipeline, TunerMixin):
    ...
    def __init__(self, args: Union[List[str], TrainArguments, None] = None) -> None:
        ...
        # 从args里面取出来tokenizer functor
        if isinstance(args, TrainArguments):
            # 在template里面注册functor
            self.template.register_tokenizer_functor(args.tokenizer_functor)
    ...
    def run(self):
        ...
        # 调用处理模型训练参数的回调
        if self.parameter_config_callback is not None:
            self.parameter_config_callback(self.model, self.tokenizer)
        trainer = trainer_cls(
            model=self.model,
    ...
    def _encode_dataset(self, train_dataset, val_dataset):
        ...
        if not is_grpo:
            if args.packing:
                ...
                print('train_dataset = LazyLLMDataset(')
                ...
                train_dataset = preprocessor(
                    ...
                    num_proc=args.dataset_num_proc,
                    load_from_cache_file=args.load_from_cache_file,
                    strict=args.strict, print_flag = True)
                if val_dataset is not None and not predict_with_generate:
